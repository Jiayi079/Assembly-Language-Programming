# Name: Xiaoqing Yao 915939354, Jiayi Gu 920024739
# Date: 12/9/2021
# Discription; CSC 256 Final Project --- An ancient robot game
...
moveRobots:
	sw $ra, -4($sp)		#store registers --> stack pointer
	sw $s0, -8($sp)
	sw $s1, -12($sp)
	sw $s4, -16($sp)
	sw $s5, -20($sp)
	sw $s6, -24($sp)
	addi $sp, $sp, -24	#update stack pointer


	li $s5,1		#  alive = 1;
	move $s2,$a2		# put x-coordinate $a2 --> $s2
	move $s3,$a3		# put y-coordniate $a3 --> $s3
...
loop: 	lw $a0,($s0)		# *ptrX = getNew(*ptrX,arg2); input we use $a0,$a1
	move $a1,$s2           	# move arg2 --> arg0 if getNew returns true
	jal getNew	        # jal getNew return value
	sw $v0,($s0)            # the return result of getNew is recorded in $v0, while the x coordinate return value is preserved in x[i]
	move $t1,$v0		# store return value of x-coordinate --> $t1 by using if statement
				
	lw $a0,($s1)		# load value *ptrY
	move $a1,$s3		# move arg3 --> arg1 of getNew;
	jal getNew	
	sw $v0, ($s1)		# *ptrY = getNew(*ptrY,arg3) store the return value of x coordinate --> y[i]
	move $t2, $v0          	# store the return value of y-coordinate in $t2 by using if statement
		
	bne $t1, $s2, inc	#    if ((*ptrX == arg2) && (*ptrY == arg3)) {
	bne $t2, $s3, inc	#.....
		
	li $s5,0                #      alive = 0;
	j endfor 	        #      break;
				#    }
inc:	addi $s0, $s0, 4	#    ptrX++;
	addi $s1, $s1, 4	#    ptrY++;
	addi $s4, $s4, 1	#    i++
	blt  $s4, $s6, loop	#  }
	...
	lw  $s6,($sp)          	# upated back the value of some registers 
	lw  $s5,4($sp)
	lw  $s4,8($sp)	
	lw  $s1,12($sp)	
	lw  $s0,16($sp)	
	lw  $ra,20($sp)      	# update back the value of $ra in order jump back to the main 
	addi $sp, $sp, 24
